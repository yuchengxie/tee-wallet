...
gPoetClient = None
gPseudoWallet = None                                                                  #伪钱包对象，用于表示当前钱包

def inputPin():                                                                       #获取tee钱包pin码函数
  psw = ''
  while True:
  ...
  #print('psw:',psw)
  #print('psw type:',type(psw))
  #print('psw len:',len(psw))
  #print('(len(psw) & 0x01)',(len(psw) & 0x01))
  if psw and (len(psw) & 0x01) == 1: psw += 'f'                                     #如果输入pin码位数是奇数，在结尾增加'f'
  return psw
...
class WalletApp(object):
  ...
  def prepare_txn1_(self, pay_to, ext_in, scan_count, min_utxo, max_utxo, sort_flag, from_uocks):                     #检查是否准备好发起转账函数
    if not self.WEB_SERVER_ADDR: return None                                                                          #如果服务器地址错误，返回None
    
    self._sequence = self._sequence + 1                                                                               #如果服务器地址正确，发起转账序列加一
    pay_from = [ protocol.format.PayFrom(0,self._wallet.address()) ]                                                  #转出钱包地址
    if ext_in:                                                                                                        #如果ext_in是一个list，按照要求格式追加到转出序列中
      for item in ext_in:
        if isinstance(item,_list_types):
          pay_from.append(protocol.format.PayFrom(item[0],item[1]))
        else: pay_from.append(item)
    if from_uocks is None:                                                                                            #如果from_uocks为None，把from_uocks赋值为同哈希值位数一样多的0
      from_uocks = [0 for item in pay_from]
      print('from_uocks', from_uocks)                                                                                 #应该为全0?
    pay_to = [protocol.format.PayTo(int(v*100000000),a) for (a,v) in pay_to]                                          #收款地址列表
    return protocol.MakeSheet(self._vcn,self._sequence,pay_from,pay_to,scan_count,min_utxo,max_utxo,sort_flag,from_uocks)        #返回一个包括所有转账操作必须的参数列表
  ...
  def submit_txn_(self, msg, submit):                                                                                          #递交转账请求函数
    headers = {'Content-Type': 'application/octet-stream'}                                                                     #设置http请求报头中的Content_Type
    r = requests.post(self.WEB_SERVER_ADDR + '/txn/sheets/sheet',data=msg.binary(self._coin.magic),headers=headers,timeout=30)  #发送一个http的post请求
    
    if r.status_code == 200:                                                                          #如果http post请求成功
      msg2 = protocol.Message.parse(r.content,self._coin.magic)                                       #接收收到的服务器回复信息
      if msg2.command == protocol.OrgSheet.command:                                                   #判断接受的服务器返回的命令是否是OrgSheet
        ...
        coin_hash = self._wallet.publicHash() + self._wallet.coin_type                                #将钱包的公钥hash和coin类型组合成coin_hash
        d = {}                                                                                        #设置一个空字典变量
        for p in msg.pay_to:
  ...
  def query_sheet(self, pay_to, ext_in=None, submit=True, scan_count=0, min_utxo=0, max_utxo=0, sort_flag=0, from_uocks=None):              #查询列表函数
    msg = self.prepare_txn1_(pay_to,ext_in,scan_count,min_utxo,max_utxo,sort_flag,from_uocks)                                               #判断是否准备好了发起转账
    if not msg: return 0                                                                                                                    #如果没有准备好，返回0
    return self.submit_txn_(msg,submit)                                                                                                     #如果准备好，发起转账
  ...
  def block_state(self, block_hash, heights=None):  # block_hash should be str or None                    #block状态信息函数
    if block_hash:                                                                                        #判断传入的block_hash参数是否有效
      if type(block_hash) != bytes:
        block_hash = block_hash.encode('latin-1')
      hash2 = hexlify(block_hash).decode('latin-1')
    else: hash2 = '00' * 32
    #print('test point:',block_hash,heights)  #测试
    if heights:
    ...
    headers = {'Content-Type': 'application/x-www-form-urlencoded'}
    account = self._wallet.address()                                  #这个参数无用？
    if type(account) == bytes:
      account2 = account.decode('latin-1')
    else: account2 = account  # account2 should be str type
    #print('account:',account)  #测试
    #print('account2:',account2)  #测试
	
    r = requests.get(self.WEB_SERVER_ADDR + '/txn/state/block',params={'hash':hash2,'hi':heights},headers=headers,timeout=30)             #向服务器发送获取数据请求
    if r.status_code == 200:
      msg = protocol.Message.parse(r.content,self._coin.magic)                                                                            #解密服务器返回的数据
      #print('r.content:',r.content)#测试
      #print('msg:',msg)#测试
      #print('self._coin.magic：',self._coin.magic)#测试
      #print('self._coin.magic type：',type(self._coin.magic))#测试
      #print('self._coin type：',type(self._coin))#测试
      #print('self._coin：',self._coin)#测试
      if msg.command == protocol.ReplyHeaders.command:
        #print('msg.command：',msg.command)#测试
        #print('protocol.ReplyHeaders.command：',protocol.ReplyHeaders.command)#测试
        #print('msg.headers：',msg.headers)#测试
        #print('msg.headers[0]__dict__：',msg.headers[0].__dict__)#测试
        if not msg.headers:                                                                              #msg.headers是一个block_headers对象索引列表
          print('no block is found!')
        else:
          for (idx,block) in enumerate(msg.headers):                                                     #注意枚举需要查询的block的信息
            hi = msg.heights[idx]
            txck = msg.txcks[idx]
  ...
  def utxo_state(self, uocks=5, address=None):  # query txns in list or num of txns
    if type(uocks) == int:                                  #判断uocks参数是一个整数？还是一个其他？获取打印UTXO的数量或者list
      num = uocks
      uocks = []
    else:
      num = 0
    ...
    if not address:                                         #判断address参数
      account = self._wallet.address()
    else: account = address
    if type(account) == bytes:                              #将address转换成str类型
      account2 = account.decode('latin-1')
    else: account2 = account  # account2 should be str type
    
    headers = {'Content-Type': 'application/x-www-form-urlencoded'}             #设置http请求报头中的'Content-Type'字段
    r = requests.get(self.WEB_SERVER_ADDR + '/txn/state/uock',params={'addr':account2,'num':num,'uock':uocks},headers=headers,timeout=30)            #发送一个http get请求
    if r.status_code == 200:                                                     #如果http请求成功，即返回状态码为200，获取相关信息
      msg = protocol.Message.parse(r.content,self._coin.magic)                   #调用nbc模块中的protocol函数来解析http请求获取的内容,返回一个nbc.protocol.messages.UtxoState对象赋值给msg
      #print(type(msg))#测试
      #print('msg: ',msg)#测试
      if msg.command == protocol.UtxoState.command:                              #如果获取数据正确，将相关信息打印出来
        for (idx,txn) in enumerate(msg.txns):
          #print('idx: ',idx)#测试
          #print('txn: ', txn, type(txn))#测试
          hi = msg.heights[idx]
          flag = msg.indexes[idx]   # flag: higher 16 bits is txns[index], lower 16 bits is out[index]
          #print('flag: ', flag,flag>>16)#测试
          ...
          for (idx2,item2) in enumerate(txn.tx_out):
            ...
            tok = script.Tokenizer(item2.pk_script)                                                     #从nbc模块中引入script函数
...
def checkPseudoWallet():                                         #用于检查当前账户钱包是否存在，如果存在就返回钱包对象，如果不存在
...
def printCmdHelp():
  ...
  info [before=uock] [after=uock]             #before和after不写时，默认值为零，表示查询当前账户所有的UTXO数据
            : account information

  block [hash=block_id] [height]+  #这个+加号是否可以去掉，容易误解
            : query block, "1-" for last one 
            ：###############################是否可以考虑增加如下示例，以便于理解
            ：for example：
            :       cmd> block hash=20ce4595356142d6c1d5c3d207ba9fa6f2495ec66f248f676c7137c94a010000 31920
            :    or cmd> block hash=20ce4595356142d6c1d5c3d207ba9fa6f2495ec66f248f676c7137c94a010000
            :    or cmd> block 31920
            ：###############################
...
def cmdLoop():
  ...
  while True:
    ...
    conn = gCard.connection           #是一个tee卡链接对象，返回smartcard.CardConnectionDecorator.CardConnectionDecorator object
    
    try:
      if sCmd == 'account':                          #account命令
        pubAddr = getPubAddr(conn)                   #conn对象的公钥地址获取函数
        pubKey = getPubKey(conn)                     #conn对象的公钥获取函数
        pubHash = getPubKeyHash(conn)                #conn对象的公钥哈希获取函数
      ...
      elif sCmd[:4] == 'info':                     #info命令
        #print('test if run to this point')#用于断点测试
        #print(checkPseudoWallet())        #用于查阅数据
        if not checkPseudoWallet():                #checkPseudoWallet()，如果返回none，表示TEE钱包还没有准备好，回到命令行
          print('TEE wallet not ready yet!')
          continue

        b = sCmd[4:].split()                       #获取info命令的参数
        
        #print('test if run to this point:',b)#用于断点测试

        uockBefore = 0; uockAfter = 0              #info命令的两个参数，初始默认值为零
        for item in b:
          bTmp = item.split('=')
          if bTmp[0] == 'before':
        ...
        sAddr = gPseudoWallet.address()            #获取当前TEE钱包地址
        
        #print('test if run to this point:%s'%(sAddr))#用于断点测试
        
        if type(sAddr) == bytes:                         
          #print('test if run to this point:',type(sAddr))#用于断点测试
          sAddr = sAddr.decode('latin-1')                #将bytes对象解码成字符串
          #print('test if run to this point:',sAddr)#用于断点测试
        print('Public address: %s' % (sAddr,))
        
        app = WalletApp(gPseudoWallet,vcn=0)                 #实例化一个钱包app
        app.WEB_SERVER_ADDR = curr_coin.WEB_SERVER_ADDR      #指定当前钱包服务器地址
        app.account_state(uockAfter,uockBefore)              #调用钱包app对象的account_state函数，查询账户状态信息。
      
      elif sCmd[:6] == 'block ':                                           #block命令
        if not checkPseudoWallet():
          print('TEE wallet not ready yet!')
          continue
        
        b = sCmd[6:].split()                                               #获取block命令参数
        
        block_hash = None                                                  #block命令参数block_hash,heights其中heights允许多个
        heights = []                                                       #如果输入非10进制数字，会引起系统报错
        for item in b:
          bTmp = item.split('=')
          if len(bTmp) == 2:
            if bTmp[0] == 'hash':
        ...
        print('test point')
        app = WalletApp(gPseudoWallet,vcn=0)                               #实例化当前WalletApp对象
        app.WEB_SERVER_ADDR = curr_coin.WEB_SERVER_ADDR                    #设置当前服务器地址
        app.block_state(block_hash,heights)                                #调用钱包app对象的block_state函数，查询区块信息。
      
      elif sCmd[:4] == 'utxo':                                     #utxo命令
        if not checkPseudoWallet():                                #检查当前钱包是否存在，如果不存在则直接返回
          print('TEE wallet not ready yet!')
          continue
        
        b = sCmd[4:].split()                                       #获取utxo命令行参数
        ...
        app = WalletApp(gPseudoWallet,vcn=0)                       #根据当前gPseudoWallet参数，实例化一个钱包对象
        app.WEB_SERVER_ADDR = curr_coin.WEB_SERVER_ADDR            #设置服务器地址
        #print(type(uocks))
        #打印一下
        print(uocks)                                               #打印一下uocks的数量
        
        app.utxo_state(uocks)                                      #调用钱包app对象的utxo_state函数，查询账户状态信息。
      
      elif sCmd[:9] == 'transfer ':                                #transfer命令
        if not checkPseudoWallet():                                #检查TEE钱包是否准备好
          print('TEE wallet not ready yet!')
          continue
        
        b = sCmd[9:].split()                                       #获取transfer命令行参数
        
        after = 0; hash_ = None
        pay_to = []
        for item in b:                                            #获取收款方账号，以及当前未转账完成交易hash
          succ = False
          bTmp = item.split('=')
          if len(bTmp) == 2:
        ...
        app = WalletApp(gPseudoWallet,vcn=0)                        #实例化一个钱包app
        app.WEB_SERVER_ADDR = curr_coin.WEB_SERVER_ADDR             #获取当前服务器地址
        
        state = ''
        txn_hash = None
        if hash_:                                                   #判断当前是否有未完成交易
          txn_hash = unhexlify(hash_)
        else:
          if not pay_to:                                            #判断收款方账号是否存在
            print('warning: pay to nobody')
            continue
          
          psw = inputPin()                                         #输入当前TEE钱包的PIN码
          if not psw: continue                                     #如果pin码错误，跳出
          gPseudoWallet.pin_code = psw                             #把输入的pin码赋值给当前钱包
          psw = ''                                                 #清空psw
          
          try:
            sn = app.query_sheet(pay_to,from_uocks=[after])        #判断钱包是否准备好
            if sn:
        ...
        try:
          ...
          if gLastErrCode[:3] == '63c':     # 63cX, authorization failed
            ...
            print('can not bind phone number to TEE second time')
        ...
        print('Notice: commands words are not standard!!!')                       #增加了提示，更便于理解
        printCmdHelp()
